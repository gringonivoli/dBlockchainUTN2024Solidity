...
contract NftMarketplace is ReentrancyGuard {
    ...
    modifier notListed(
        ...
        NFTInfo memory listing = s_listings[nftAddress][tokenId];
        if (listing.price > 0) {
     ...
     */
    function listNFT( // no listed;who listed must be owner;price > 0;NFT approved for market contract;
        address nftAddress,
        uint256 tokenId,
        uint256 price
    )
     ...
     * @param tokenId Token ID of NFT
     */
    function cancelNFT(address nftAddress, uint256 tokenId) // who cancel must be owner; is listed;
        external
        isOwner(nftAddress, tokenId, msg.sender)
        ...
        nonReentrant
    {
        ...
        NFTInfo memory listedItem = s_listings[nftAddress][tokenId];
